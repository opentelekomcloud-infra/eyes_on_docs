...
print("**GITHUB SWISS INFO SCRIPT IS RUNNING**")
...
def extract_pull_links(cur):
    ...
    try:
        cur.execute('SELECT "Auto PR URL" FROM orphaned_prs_swiss;')
...
def add_github_columns(cur, conn):
    ...
    try:
        cur.execute(
            '''
            ALTER TABLE orphaned_prs_swiss
...
def update_orphaned_prs(cur, conn, rows, auto_prs):
    ...
    for row in rows:
        pr_id, pull_link = row
        gitea_repo_name = re.search(r"/docs-swiss/(.+?)/", pull_link).group(1)
...
def main():
    ...
    cur.execute('SELECT id, "Auto PR URL" FROM orphaned_prs_swiss;')
    rows = cur.fetchall()
