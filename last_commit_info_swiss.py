...
print("**LAST COMMIT SWISS INFO SCRIPT IS RUNNING**")
        ...
        try:
            ...
            for path in {doctype}:
                try:
                    ...
                    cur.execute(
                        'INSERT INTO last_update_commit_swiss ("Service Name", "Doc Type", "Last commit at", "Days passed", "Commit URL") VALUES (%s, %s, %s, %s, %s);',
...
def update_squad_and_title(conn, cur):
    ...
    try:
        cur.execute("SELECT * FROM last_update_commit_swiss;")
        open_issues_rows = cur.fetchall()

        for row in open_issues_rows:
            cur.execute(
                """UPDATE last_update_commit_swiss
                    SET "Service Name" = rtc."Title", "Squad" = rtc."Category"
                    FROM repo_title_category AS rtc
                    WHERE last_update_commit_swiss."Service Name" = rtc."Repository"
                    AND last_update_commit_swiss.id = %s;""",
                (row[0],)
            )
            cur.execute(
                """UPDATE last_update_commit_swiss
                    SET "Squad" = 'Other'
                    WHERE last_update_commit_swiss."Service Name" IN ('doc-exports', 'docs_on_docs', 'docsportal')
                    AND last_update_commit_swiss.id = %s;""",
                (row[0],)
            )
            conn.commit()
...
def main():
    ...
    conn = connect_to_db()
    cur = conn.cursor()
    cur.execute("DROP TABLE IF EXISTS last_update_commit_swiss")
    create_commits_table(conn, cur, "last_update_commit_swiss")
