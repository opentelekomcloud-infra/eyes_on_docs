...
print("**OPEN ISSUES SWISS SCRIPT IS RUNNING**")

gitea_api_endpoint = "https://gitea.eco.tsi-dev.otc-service.com/api/v1"
session = requests.Session()
session.debug = False

gitea_org = "docs-swiss"
github_org = "opentelekomcloud-docs-swiss"
...
def create_open_issues_table(conn, cur):
    try:
        cur.execute(
            '''CREATE TABLE IF NOT EXISTS open_issues_swiss (
        ...
        conn.commit()
        print(f"Table open_issues_swiss has been created successfully")
    except psycopg2.Error as e:
        print(f"Tables creating: an error occurred while trying to create a table open_issues_swiss in the database: {e}")
...
def get_issues_table(gitea_issues, github_issues, cur, conn):
    ...
    try:
        for tea in gitea_issues:
            ...
            if 'assignees' in tea and tea['assignees'] is not None:
                ...
                assignees = ''
            cur.execute('INSERT INTO open_issues_swiss ("Environment", "Service Name", "Squad", "Issue Number", "Issue URL", "Created by", "Created at", "Duration", "Comments", "Assignees") VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)',
    ...
    service_pattern = re.compile(r"(?P<service_name>(?<=\/opentelekomcloud-docs-swiss\/)([^\/]+)(?=\/))")
...
def update_squad_and_title(conn, cur):
    ...
    try:
        cur.execute("SELECT * FROM open_issues_swiss;")
        open_issues_rows = cur.fetchall()

        for row in open_issues_rows:
            cur.execute(
                """UPDATE open_issues_swiss
                    SET "Service Name" = rtc."Title", "Squad" = rtc."Category"
                    FROM repo_title_category AS rtc
                    WHERE open_issues_swiss."Service Name" = rtc."Repository"
                    AND open_issues_swiss.id = %s;""",
                (row[0],)
            )
            cur.execute(
                """UPDATE open_issues_swiss
                    SET "Squad" = 'Other'
                    WHERE open_issues_swiss."Service Name" IN ('doc-exports', 'docs_on_docs', 'docsportal')
                    AND open_issues_swiss.id = %s;""",
                (row[0],)
            )
            conn.commit()
...
def main():
    ...
    org = g.get_organization("opentelekomcloud-docs-swiss")
    ...
    cur.execute(
        f'''DROP TABLE IF EXISTS open_issues_swiss'''
    )
    conn.commit()
